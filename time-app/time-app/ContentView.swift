...
struct CountdownView: View {
    ...
    @State private var showAlert = false // 控制浮窗显示的状态
    ...
    var body: some View {
        ...
        .alert(isPresented: $showAlert) {
            Alert(
                title: Text("倒计时结束"),
                message: Text("点击按钮停止铃声"),
                dismissButton: .default(Text("停止铃声")) {
                    stopAlarm()
                }
            )
        }
...
struct TimerView: View {
    ...
    func playSelectedMusic() {
        ...
        guard let url = Bundle.main.url(forResource: musicFile, withExtension: nil, subdirectory: "music") else {
            print("音乐文件未找到: \(musicFile)")
            return
        }
        do {
            audioPlayer = try AVAudioPlayer(contentsOf: url)
            ...
            print("播放音乐时发生错误: \(error.localizedDescription)")
        }
    }
